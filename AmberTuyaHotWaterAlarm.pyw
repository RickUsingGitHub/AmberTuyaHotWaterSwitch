...
class AmberAlarmApp:
    ...
    def load_settings(self):
        settings_file = config.get("SETTINGS_FILE", "amber_alarm_settings.json")
        if os.path.exists(settings_file):
            try:
                with open(settings_file, 'r') as f:
                    settings = json.load(f)
                    self.low_threshold = settings.get('low_thresh', self.low_threshold)
                    self.high_threshold = settings.get('high_thresh', self.high_threshold)
            except Exception as e:
